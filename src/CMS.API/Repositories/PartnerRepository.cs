...
public sealed class PartnerRepository(IDbConnectionFactory connectionFactory) : IPartnerRepository
{
    ...
    {
        // No transaction: Partner has no junction rows to keep in step, so this is a single
        // statement (contrast AppRoleRepository, which must span the AppUserRole sync).
        // SCOPE_IDENTITY() returns numeric(38,0), hence the CAST to the pkid's own type.
        const string sql = """
    ...
    public async Task<bool> DeleteAsync(short pkid, CancellationToken ct = default)
    {
        ...
        // propagate: the controller turns it into a 409 rather than silently orphaning content.
