...
/// (lab05 Step 5). These run through the real JWT middleware via <see cref="AppUsersAuthTestFactory"/>
...
/// </summary>
public sealed class AppUsersAuthorizationTests : IClassFixture<AppUsersAuthTestFactory>
{
    private readonly AppUsersAuthTestFactory _factory;

    public AppUsersAuthorizationTests(AppUsersAuthTestFactory factory) => _factory = factory;

    private HttpClient ClientFor(params string[] roles)
    {
        var token = JwtTokenGenerator.Generate(
            AppUsersAuthTestFactory.SigningKey, "caller", "Caller", roles);
